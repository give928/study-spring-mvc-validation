...
import com.give928.spring.mvc.itemservice.domain.item.AddCheck;
import com.give928.spring.mvc.itemservice.domain.item.EditCheck;
...
//    @PostMapping("/add")
    ...
    @PostMapping("/add")
    public String addItemV2(@Validated(AddCheck.class) @ModelAttribute Item item,
...
//    @PostMapping("/{itemId}/edit")
    ...
    public String editV2(@PathVariable Long itemId,
                         @Validated(EditCheck.class) @ModelAttribute Item item,
                         BindingResult bindingResult) {
        //특정 필드 예외가 아닌 전체 예외
        if (item.getPrice() != null && item.getQuantity() != null) {
            int resultPrice = item.getPrice() * item.getQuantity();
